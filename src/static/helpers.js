...
function loadData(fileName) {
  var url = 'http://localhost:3000/data/' + fileName + '.json';
...
function loadDataIntoName(name) {
  return new Promise(function (resolve, reject) {
    loadData(name).then(function (data) {
...
function loadMaterialColors() {
  return new Promise(function (resolve, reject) {
    loadData('material').then(function (data) {
      materialColors = data;
      resolve();
    });
  });
...
function loadReportText() {
  return new Promise(function (resolve, reject) {
    loadData('report_text').then(function (data) {
      reportText = data;
      resolve();
    });
  });
...
function drawDashedChartLines(chart, yScale, chartWidth) {
  chart.append('line').attr('x1', 0).attr('y1', function () {
    return yScale(30);
  }).attr('x2', chartWidth).attr('y2', function () {
    return yScale(30);
  }).attr('stroke', 'black').attr('stroke-width', 1).attr('stroke-dasharray', '6, 3');
...
function drawColumnBorders(chartGroup, columnNames, xScale, yScale) {
  var lineGroups = chartGroup.selectAll('.border').data(columnNames, function (d) {
    return d;
  });
  var exitingLineGroup = lineGroups.exit();
  ...
  exitingLineGroup.select('.left-line').transition().duration(250).attr('y1', function (d) {
    return yScale(0);
  });
  exitingLineGroup.select('.right-line').transition().duration(500).attr('y1', function (d) {
    return yScale(0);
  });
  ...
  var lineGroup = lineGroups.enter().append('g').attr('class', 'border').attr('transform', function (d) {
    return 'translate(' + xScale(d) + ',0)';
  });
  lineGroup.append('line').attr('class', 'left-line').attr('x1', 0).attr('y1', function (d) {
    return yScale(100);
  }).attr('x2', 0).attr('y2', function (d) {
    return yScale(0);
  }).attr('stroke', '#000000').attr('stroke-width', 1);
  lineGroup.append('line').attr('class', 'top').attr('x1', -3).attr('y1', function (d) {
    return yScale(100);
  }).attr('x2', 3).attr('y2', function (d) {
    return yScale(100);
  }).attr('stroke', '#000000').attr('stroke-width', 1);
  lineGroup.append('line').attr('class', 'right-line').attr('x1', xScale.bandwidth()).attr('y1', function (d) {
    return yScale(100);
  }).attr('x2', xScale.bandwidth()).attr('y2', function (d) {
    return yScale(0);
  }).attr('stroke', '#000000').attr('stroke-width', 1);
  lineGroup.append('line').attr('class', 'top').attr('x1', xScale.bandwidth() - 3).attr('y1', function (d) {
    return yScale(100);
  }).attr('x2', xScale.bandwidth() + 3).attr('y2', function (d) {
    return yScale(100);
  }).attr('stroke', '#000000').attr('stroke-width', 1);
}
