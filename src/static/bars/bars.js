...
var selectionGroup = svg.append('g').attr('transform', 'translate(' + margin.left + ',' + (chartGroupHeight + margin.top + 10) + ')');
...
var xAxisGroup = chartGroup.append('g').attr('class', 'axis axis--x').attr('transform', 'translate(0,' + chartGroupHeight + ')');
...
function loadBarsPage() {
  ...
  selectionGroup = svg.append('g').attr('transform', 'translate(' + margin.left + ',' + (chartGroupHeight + margin.top + 10) + ')');
  selectedCategory = '';
  selectedFacet = '';
  ...
  xAxisGroup = chartGroup.append('g').attr('class', 'axis axis--x').attr('transform', 'translate(0,' + chartGroupHeight + ')');
  yAxisGroup = chartGroup.append('g').attr('class', 'axis axis--y');
}
...
function drawFacetData(category, facet) {
  var data = pullOutFacetData(category, facet);
  xScale.padding([0.2]);
  drawRectGroups(data);
}
...
function drawRectGroups(data) {
  ...
  var enteringRectGroups = rectGroups.enter().append('g').attr('class', 'groups').attr('transform', function (d) {
    return 'translate(' + xScale(d.name) + ',' + 0 + ')';
  });
  rectGroups.selectAll('rect').data(function (d) {
    return d.dates;
  }, function (d) {
    return d.date;
  ...
  enteringRectGroups.selectAll('rect').data(function (d) {
    return d.dates;
  }, function (d) {
    return d.date;
  }).enter().append('rect').attr('x', 0).attr('fill', function (d) {
    var color = d3.color(d.color);
    return color + '';
  }).attr('stroke-width', 1).attr('stroke', 'black').attr('y', function (d) {
    return yScale(0);
  }).attr('width', function (d, i) {
    return d.scale.bandwidth() * (d.length - i);
  }).attr('height', function (d) {
...
function pullOutFacetData(category, facet) {
  ...
  var _loop = function () {
    ...
    for (var i = 0; i < o.dates.length; i++) {
      o.dates[i].scale = d3.scaleBand().domain(o.dates.map(function (date) {
        return date.date;
...
function drawFacetNames(category) {
  var facetNames = pullOutFacetNames(category);
  facetNames.unshift('Overall');
  var spans = d3.select('#facetsDiv').selectAll('span').data(facetNames, function (d) {
    return d;
  });
  spans.exit().remove();
  ...
  spans.enter().append('span').attr('class', function (d, i) {
    if (i == 0) {
      return 'underline';
    } else {
      return '';
