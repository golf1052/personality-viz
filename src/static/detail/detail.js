...
var selectedCategory = 'Extraversion';
var selectedUser = '';
var selectedDate = '';

var extra = d3.select('#Extraversion');
var agree = d3.select('#Agreeableness');
var consci = d3.select('#Conscientiousness');
var neuro = d3.select('#Neuroticism');
var open = d3.select('#Openness');
...
function setup() {
  var dates = pullOutDates(selectedUser);
  drawDateInputs(dates);
  var input = d3.select('.date:last-child').select('input');
...
function displayScores() {
  reportText.categories.forEach(function (cat) {
    if (cat.category == selectedCategory) {
      (function () {
        var categoryDiv = d3.select('#categoryDiv');
        categoryDiv.select('#details').select('p').text(cat.details);
        var userResults = names[selectedUser].data.reports.find(function (report) {
...
function pullOutDates(userName) {
  return names[userName].data.reports.map(function (report) {
    return report.date;
  });
}
