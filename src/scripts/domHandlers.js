...
domReady( () => {
  ...
  */
  ( function selectToggle() {
    d3.select('.select__option--default')
      .on( 'click', () => {
        if ( d3.select('.select-options-container').classed('select-options-container--open') ) {
          d3.select('.select-options-container--open')
          .classed( 'select-options-container--open', false )
        }
        else {
          d3.select('.select-options-container')
          .classed( 'select-options-container--open', true )
        }
      });
  } () );
