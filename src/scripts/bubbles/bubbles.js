...
var selectionGroup = svg.append('g')
  .attr('transform', 'translate(' + (chartGroupWidth + margin.right) + ',0)');
...
allDataPromise.then(function(allData) {
  ...
  var selectGroup = selectionGroup.selectAll('g').data(pullOutSelectionData()).enter()
    .append('g')
    .attr('id', function(d) {
      return d.key;
    })
  ...
  var selectRects = selectGroup.append('rect')
    .attr('x', 0)
    .attr('y', function(d, i) {
      return 0;
    })
    ...
    for (var i = 0; i < names[name].data.reports.length; i++) {
      ...
      overallData.results.forEach(function(cat) {
        if (cat.category == category) {
          for (let i = 0; i < Object.keys(cat.facets).length; i++) {
            let key = Object.keys(cat.facets)[i];
            values.push({
              key: key,
              date: overallData.date,
